...
pub fn update(points: *[window.CAPACITY]particle.Particle) !void {
    ...
    for (0..window.WINDOW_WIDTH) |x| {
        for (0..window.WINDOW_HEIGHT) |y| {
            ...
            if (points[xy_i(x, y)] != particle.Particle.empty and y != window.WINDOW_HEIGHT - 1) {
                if (points[xy_i(x, y + 1)] == particle.Particle.empty) {
                    points[xy_i(x, y + 1)] = particle.Particle.sand;
                    points[xy_i(x, y)] = particle.Particle.empty;
                }
            }
        }
    }
